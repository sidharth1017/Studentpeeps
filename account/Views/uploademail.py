...
class VerifyUploadEmailUsers(View):
    ...
    def get(self, request):
        return render(request, 'admin/userVerify.html')
    ...
    def post(self, request):
        ...
        try:
            ...
            if not register.is_verified:
                ...
                messages.append("User is now verified and welcome SMS sent to their phone.")
            else:
                messages.append("User is already verified!")

        except Register.DoesNotExist:
            try:
                unverified_user = UnVerifiedIdUpload.objects.get(phone=phone)
                user = unverified_user.user
                register = Register(
                    user=user,
                    phone=unverified_user.phone,
                    firstname=unverified_user.firstname,
                    lastname=unverified_user.lastname,
                    gender=unverified_user.gender,
                    birthday=unverified_user.birthday,
                    collegeId=unverified_user.collegeId
                )
                register.is_verified = True
                register.save()
...
class RejectUploadPhoneUsers(View):
    def post(self, request):
        messages = [] 
        phone = request.POST['phone']
        register = None
        ...
        try:
            ...
            try:
                register = UnVerifiedIdUpload.objects.get(phone=phone)
                user = register.user
                messages.append("User is rejected from UnVerifiedIdUpload!")             
            except UnVerifiedIdUpload.DoesNotExist:
                messages.append("User not found!")
        ...
        messages.append("SMS sent to user about rejection and user is now inactive.")
        return render(request, 'admin/userVerify.html', {'messages': messages})
